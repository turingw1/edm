...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render a dense high-quality DG-TWFD sample gallery.")
    ...
    parser.add_argument("--steps", required=True, type=int, help="Actual step count used for generation.")
    ...
    parser.add_argument("--manifest", default=None, help="Optional fixed-row manifest JSON for conditional galleries.")
    parser.add_argument("--seeds", default=None, help="Comma/range list of seeds when --manifest is not used.")
    parser.add_argument("--class-idx", type=int, default=None, help="Optional fixed class id when --manifest is not used.")
    parser.add_argument("--grid-cols", type=int, default=8, help="Number of columns in the gallery.")
    ...
    if args.manifest:
        ...
        if not args.seeds:
            raise ValueError("Either --manifest or --seeds must be provided")
    ...
    payload = {
        ...
        "image_grid_layout": {"rows": int((len(resolved_rows) + args.grid_cols - 1) / args.grid_cols), "cols": int(args.grid_cols), "cell_size": cell_size},
        "raw_output_root": str(sample_dir),
        "generation": {key: value for key, value in stats.items() if key != "resolved_rows"},
        "note": "Dense high-quality gallery for visual inspection after the main candidate figures are generated.",
    }
